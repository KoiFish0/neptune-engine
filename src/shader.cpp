...
static int compileShader(unsigned int type, const std::string& source) {
  ...
  const char* src = source.c_str(); // source code to be loaded into the shader
...
RGB colorConverter(const std::string& hexValue) {
    RGB rgbColor;
    int r, g, b;
...
// Create shader with dynamic color from hex string
unsigned int colorToShader(const std::string& hexValue) {
    RGB rgbColor = colorConverter(hexValue);
...
// Create text shader with dynamic color from hex string
unsigned int colorToShader_Text(const std::string& hexValue) {
    // TODO Fix aspect ratio related issues. Need proper projection matrix
    RGB rgbColor = colorConverter(hexValue);
    ...
    // Create the fragment shader with given rgb values 
    std::stringstream fragSrc;
    fragSrc << "#version 330 core\n"
            << "in vec2 TexCoords;\n"
            << "out vec4 color;\n"
            ...
            << "color = vec4(" << rgbColor.r << "," << rgbColor.g << "," << rgbColor.b << ", 1.0) * sampled;\n"
            << "}\n";
